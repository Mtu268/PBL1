...
struct Book {
	...
	int publishYear;
	int used;
	int status; //Br � th? l� "Book Reserved" (s�ch dang du?c mu?n ho?c d?t tru?c), ho?c m?t bi?n ph? d? d�nh d?u tr?ng th�i n�o d� trong chuong tr�nh (t�y b?n quy d?nh).
    int Amount;
} ;
...
// T�m kiem s�ch theo t�n t�c gia
void findAuthor(Node head) {
    char author[100];
...
// T�m ki?m s�ch theo nh� xuat ban
void findPublisher(Node head) {
    char publisher[100];
